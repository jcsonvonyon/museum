...
export default function SectionQuote() {
    ...
    return (
        <section ref={ref} className="safe-x-padding my-[172px]" aria-label='Quote Section'>
            <div className='relative'>
                <motion.p initial={{ y: 100, opacity: 0 }} animate={inView ? { y: 0, opacity: 1 } : {}} transition={{ duration: 0.5 }} className="font-medium text-base md:text-xl leading-[38px] lg:text-2xl text-accent max-w-[933px] mx-auto text-justify">
                    This Website was made with heart and soul, pouring passion and creativity into every detail to bring it to life. It&apos;s not just a website , it&apos;s a labor of love.
                </motion.p>

                {animatedFloatingImages.map((item, index) => (
                    <img key={index.toString()} {...item} />
                ))}
            </div>
            <motion.p initial={{ y: 100, opacity: 0 }} animate={inView ? { y: 0, opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.5 }} className="font-medium text-6xl lg:text-7xl text-accent text-center mt-[20px]" >
                Claudia
            </motion.p>
        </section >
    )
